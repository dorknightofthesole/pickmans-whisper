...
- [ ] **P4** — At max stage (4 / Black Putrefaction), toast urging the player to eat her before she is too ripe. Cannibal-perk gated (`FID_PERK_CANNIBAL_1/2/3`, any rank), ModConfig `eatRipeCorpseToast` ({name} → Victim override name, or "her" if unnamed), shared once-per-game-hour cooldown across all ripe corpses. Checked every ambient KillerScan sweep (not just on the stage-4 transition) via `CorpseDecayScript.SyncOverlaysFromKillerScanSnapshot` → `MainQuestScript.MaybeToastEatRipeCorpse`. Coded — awaiting in-game confirm (needs Cannibal perk + a corpse aged to Black Putrefaction).
