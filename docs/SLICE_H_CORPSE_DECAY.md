...
- [x] **P2** — **Deliver working Corpse Decay stage change in MCM** (Victims Set / Reset = kill clock only; KillerScan → `SyncOverlaysFromKillerScanSnapshot` applies). Stage 0 body `none`; Pallor (1) = tinted, multi-template `SkinTexture_16+SkinTexture_09+SkinTexture_18`. Verified in-game — MCM path confirmed "damn near perfect" coverage; ambient KillerScan dispatch re-enabled with simplified want-vs-last logic (see below).
...
- [x] **P4** — At max stage (4 / Black Putrefaction), toast urging the player to eat her before she is too ripe. Cannibal-perk gated (`FID_PERK_CANNIBAL_1/2/3`, any rank), ModConfig `eatRipeCorpseToast` ({name} → Victim override name, or "her" if unnamed), shared once-per-game-hour cooldown across all ripe corpses. Checked every ambient KillerScan sweep (not just on the stage-4 transition) via `CorpseDecayScript.SyncOverlaysFromKillerScanSnapshot` → `MainQuestScript.MaybeToastEatRipeCorpse`. Verified in-game.
- [x] **P5** — Detect the player actually eating a max-stage corpse and react. No vanilla FO4 script registers an animation event for the Cannibal eat action (checked the real Scripts/Source — Bloodbug's feed event is the only precedent, and it uses an arbitrary unguessable per-animation name), so detection instead listens for the vanilla `PerkCannibalHeal` spell's `RestoreHealthGeneric` MGEF landing on the player (`FID_MGEF_RESTORE_HEALTH_GENERIC` verified against Fallout4.esm, 5s duration). A Quest-level `RegisterForMagicEffectApplyEvent(PlayerRef, ...)` was tried first and confirmed dead live (an unfiltered sniff caught nothing over two minutes of play); registration moved to `PickmansWhisperPlayerAliasScript` (ReferenceAlias filled with the player) — `RegisterForMagicEffectApplyEvent(Self, ...)` there, local `Event OnMagicEffectApply(ObjectReference akTarget, ObjectReference akCaster, MagicEffect akEffect)` (all 3 params required — Caprica rejects a shortened 2-param override some community snippets use). That effect is shared with Stimpaks etc., so `MainQuestScript.MaybeRewardEatenRipeCorpse` disambiguates by requiring the Cannibal perk (impossible to have triggered the heal from eating otherwise) plus the nearest `KillerScan().ScanDead` corpse (capped at `BUTCHER_CORPSE_RADIUS`, same "which of several nearby corpses" pattern as `GetFacedSeverCorpse`) being tracked and at max decay stage. ModConfig `ateRipeCorpseToast` ({name} → "She" if unnamed) fires on success — verified in-game twice (named victims resolved correctly both times). Bonus: `ApplyEatRipeCorpseBonus` grants an END buff via the new dedicated `PickmansWhisperBuffTrackerScript` (attached to the Main quest VMAD, extensible for future buffs) — amount/cap/duration from ModConfig `ateRipeCorpseEndBuffAmount/MaxDelta/Hours` (shipped 2/4/2h), clamped to a max total delta rather than stacking unbounded, single whole-buff expiry timer refreshed on each eat, ticked from `KillerScanScript.DispatchListeners` (no timer of its own). Coded — bonus awaiting in-game confirm.
...
**MCM override: `bDecayMissingLimbs:Victims` (default OFF).** Sits next to `bDecayVisuals:Victims` on the Decay stage options section. Off (default) keeps the stump-halo protection above exactly as-is. On, `ApplyDecayStageOverlays` treats a dismembered corpse exactly like a fully-limbed one — computed once as `bypassLimbGate = abForcePaint || IsDecayMissingLimbsAllowed()` and reused for `limbsIntact` and every `IsDismembered("Head1")` gate (body paint, face-first, face re-equip, and the final status check), so body decals and the face mask both apply regardless of missing limbs/head when it's on.
...
**Ambient KillerScan dispatch — disabled during the MCM-trigger debugging above, now re-enabled with simplified logic.** Was commented out (not deleted) while the render pipeline and MCM trigger issues below were diagnosed — see RESOLVED paragraphs. Once the MCM path was confirmed working end-to-end in-game, `KillerScanScript.DispatchListeners()`'s call to `CorpseDecay().CallFunctionNoWait("SyncOverlaysFromKillerScanSnapshot", None)` (every 4th tick) was restored. The function itself was simplified in the same pass: it no longer owns its own rate-limit (`LastOverlaySyncReal`/`OVERLAY_SYNC_MIN_SECONDS`) or failure backoff (`DecaySyncBackoffUntil`, `NoteDecayClockChangedForSync`) — both removed as dead complexity now that the actual per-corpse decision is just "does `ResolveDecayStageForKill` differ from `GetDecayKillLastStage`," delegated entirely to `SyncDecayForKnifeCorpse` (the exact function the MCM Set/Reset path calls — same apply, same no-op-on-match skip). The only remaining gates are the tick throttle above and the shared `OverlaySyncBusy` reentrancy watchdog (still needed — `CallFunctionNoWait` + LooksMenu `Utility.Wait` can re-enter this script). Untracked knife-kill victims are still auto-stamped Freshly Deceased on first ambient sweep via `EnsureDecayForTrackedVictim`. Bed Gift is unaffected (calls `ApplyBedGiftDecayOverlays` directly).
